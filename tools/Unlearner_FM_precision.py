...
class Unlearner_FM_precision(Module):
    ...
    def Fisher_Masking(self,retain_dataloader:DataLoader,forget_dataloader:DataLoader):
            ...
            named_layers=Unlearner_FM_precision.get_named_layers(self.model,is_state_dict=False)

            ### get Hessain wrt to Forget dataloader
            self.forget_hess=Unlearner_FM_precision.Hessian(forget_dataloader,self.model,self.device)
            ...
            if retain_dataloader is not None:
                ...
                self.retain_hess=Unlearner_FM_precision.Hessian(retain_dataloader,self.model,self.device)
            ...
            named_layers=Unlearner_FM_precision.get_named_layers(self.model)
    ...
    def Hessian(dataloader:DataLoader,model:Module,device:str):
          ...
          if next(model.parameters()).is_cuda:
                        ...
                        for param in model.parameters():
                            print('Parameter Type check',param.dtype)
                            break
                        ...
                        model=model.half()
                        for param in model.parameters():
                            print('Parameter Type check when pushed to GPU',param.dtype)
                            break
